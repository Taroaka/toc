...
- Use `python3 ./kindle/extract-kindle-web-cdp.py --run-dir "__RUN_DIR__" --pages 5 --port 59708 --ocr-mode none`.
- After image export succeeds, run `./kindle/transcribe-kindle-pages-with-codex.sh "__RUN_DIR__"`.
- If the extractor reports that the reader is not open yet, stop and tell me what it needs.
- Record the result in `__SESSION_PATH__`.
...
4. Read `__TRANSCRIPT_PATH__` and `__SESSION_PATH__`.
