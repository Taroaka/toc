...
- 各カットの画像は毎回新規生成しなくてよい。必要なのは、同一 run 内で同じ場所/物体/人物状態をまたぐ continuity anchor を作るか再利用するかの判断
...
- `scenes[].cuts[]` として、各cutを `image_generation` に落とし込んだ（新規生成が不要な cut は同一 run 内の既存 anchor の再利用でよい）
