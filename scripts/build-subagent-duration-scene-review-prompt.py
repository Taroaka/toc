#!/usr/bin/env python3
"""Build a pasteable prompt for contextless scene-duration review."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a pasteable prompt for contextless scene-duration review.")
    ...
    prompt = build_duration_scene_review_prompt(
        run_dir=run_dir,
        minimum_seconds=int(args.min_seconds),
        actual_seconds=int(args.actual_seconds),
        flow=args.flow,
    )
    prompt_path = write_review_prompt(run_dir=run_dir.resolve(), kind="scene", prompt=prompt)
    append_state_snapshot(
        run_dir.resolve() / "state.txt",
        {
            "review.duration_fit.scene_prompt": str(prompt_path.relative_to(run_dir.resolve())),
