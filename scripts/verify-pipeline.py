...
from toc.stage_evaluation.pipeline import (  # noqa: E402
...
from toc.stage_evaluation.video import _probe_duration  # noqa: E402
...
def check_manifest_single(run_dir: Path, profile: str, flow: str) -> tuple[dict[str, Any], dict[str, str]]:
    return pipeline_policy.check_manifest_single(run_dir, profile, flow)


def check_manifest_scene_series(run_dir: Path, profile: str) -> tuple[dict[str, Any], dict[str, str]]:
    return pipeline_policy.check_manifest_scene_series(run_dir, profile)
...
def _video_checks(checks: list[dict[str, Any]], *, video_path: Path, state: dict[str, str], run_dir: Path) -> None:
    pipeline_policy.append_video_checks(
        checks,
...
def check_video_single(run_dir: Path, *, target_slot: str = "p930") -> tuple[dict[str, Any], dict[str, str]]:
    return pipeline_policy.check_video_single(
        run_dir,
        append_semantic_review=append_semantic_review_check,
        duration_probe=_probe_duration,
        target_slot=target_slot,
...
def check_video_scene_series(run_dir: Path, *, target_slot: str = "p930") -> tuple[dict[str, Any], dict[str, str]]:
    return pipeline_policy.check_video_scene_series(
        run_dir,
        append_semantic_review=append_semantic_review_check,
        target_slot=target_slot,
    )
...
def _supervisor_result_issues(
    ...
    *,
    ...
    required_target_slot = None
    target_number = _slot_number_from_code(stage_target)
    if bucket == "p600" and target_number is not None and target_number >= 680:
        required_target_slot = "p680"
        if (
            not isinstance(completed_slots, list)
            or required_target_slot not in completed_slots
        ):
            issues.append(
                f"{bucket}:target_slot_missing:{required_target_slot}"
            )
    ...
    if not isinstance(state_keys, dict):
        ...
        if (
            required_target_slot is not None
            and f"slot.{required_target_slot}.status" not in state_keys
        ):
            issues.append(
                f"{bucket}:target_slot_state_key_missing:"
                f"{required_target_slot}"
            )
