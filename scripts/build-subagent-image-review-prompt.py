...
def build_subagent_image_review_prompt(*, run_dir: Path, flow: str | None = None) -> str:
    ...
    lines = [
        ...
        "- Does each scene still read as the visible initial state of the later video clip, not as mid-action or a completed action?",
        "- Do the prompt bodies avoid first-frame authoring metadata such as `最初の1フレーム`, `1フレーム目`, or `first frame`?",
        "- Do the prompt bodies avoid nonvisual production metadata such as `物語「<topic>」の scene10`, `scene10_cut01`, or `[物語の文脈]`?",
        "- Are any prompts technically valid but artistically weak, generic, or over-constrained?",
        "- Does every production scene expose at least two concrete cuts in `video_manifest.md.scenes[].cuts[]`, with distinct image prompts and outputs for each cut?",
        ...
        "- any production scene has no `cuts` array or fewer than two cuts",
        "- cut entries are collapsed into scene-level image_generation instead of `scenes[].cuts[].image_generation`",
        "- reveal order violation",
        "- main subject drift",
        "- missing core character/setpiece relationship",
        "- prompt too generic to preserve the intended subject",
        "- prompt is not first-frame ready for the later video clip",
        "- prompt contains first-frame authoring metadata instead of visible-state wording",
        "- nonvisual production metadata instead of concrete visual subject wording",
        "",
