...
p500/p600 の Codex built-in image generation は production では直列実行する。generated-images fallback は `turn/completed` より先に共有ディレクトリから画像を拾うことがあり、複数 request を並列実行すると「完了した別 request の画像」を現在の output に保存して asset ID と画像内容がずれる。画像保存 provenance / 並列制御の問題は semantic QA ではなく deterministic output validator と frontend human review の診断として扱う。
