...
フロントから物語を1から作る Image generation app の create flow は、server 都合の最短 scaffold として実装しない。人間承認だけを frontend handoff にし、それ以外の grounding、deterministic verifier、review-loop prompt/report/aggregate materialization、story/image prompt consistency review、asset/scene request materialization、asset/scene image generation、p650/p680 validation は通常経路を通す。`server/image_gen_app.py` はこの orchestration を呼び出す入口に留め、品質 gate や artifact 契約を迂回する独自ショートカットを持たない。frontend create は backend process から `scripts/toc-immersive-frontend-run.py` を直接実行し、`toc-immersive-runner` skill を app-server turn 内で起動してさらに app-server/image/semantic 処理を呼ぶ nested app-server 経路を使わない。理由は、外側の app-server sandbox と内側の generation runtime で `CODEX_HOME` / generated image root / network diagnostics が分裂し、asset/scene 画像が作られない失敗を restart health check で検出できなくなるため。
...
p500/p600 の Codex built-in image generation は production では直列実行する。generated-images fallback は `turn/completed` より先に共有ディレクトリから画像を拾うことがあり、複数 request を並列実行すると「完了した別 request の画像」を現在の output に保存して asset ID と画像内容がずれる。semantic QA がファイル名と中身の不一致を検出した場合は制作意味の修正ではなく、画像保存 provenance / 並列制御の問題として扱う。

p500/p600 の Codex built-in image generation は 1 枚あたりの既定 timeout を長めに取る。app-server image turn は 300 秒を超えて正常完了することがあるため、production では `TOC_IMAGE_GEN_ITEM_TIMEOUT_SECONDS=900` を標準にし、短い timeout による不要な retry で provenance を不明瞭にしない。
