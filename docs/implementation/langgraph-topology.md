...
- authoring 直後の review slot は単発 gate ではなく、最大 5 round の evaluator-improvement loop として扱う
  - 1 round は 5 critic agents + 1 aggregator agent
  - critic / aggregator は report artifact だけを出力し、canonical artifact と `state.txt` は統括エージェントだけが更新する
...
- evaluator-improvement loop の修正適用は統括エージェントが single writer として行う
