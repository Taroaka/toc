...
対象 run（現在作業中の run に置き換える。別の `output/*` run を生成成果物・fallback・成功根拠として参照しない）:

- `output/<topic>_<timestamp>/`

test spec:

- `output/<topic>_<timestamp>/assets/test/scene01-built-in-test-batch-spec.md`
- `output/<topic>_<timestamp>/assets/test/scene01-built-in-image-prompts.md`
...
python scripts/import-codex-generated-image.py \
  --dest "output/<topic>_<timestamp>/assets/test/scene01_cut01__built_in_test.png"
```
