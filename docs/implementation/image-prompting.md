...
ただし、**全scene/cutに新規の生成静止画が必要とは限らない**。新規生成は、同一 run 内で同じ場所/物体/人物状態の continuity anchor を初めて作るとき、または複数scene/cutで再利用する参照画像が必要なときに優先する。
...
0.1. no-reference image request は `execution_lane=bootstrap_builtin` として built-in lane に寄せる
...
- 参照画像は、毎回新規に作るのではなく、同一 run 内で同じ場所/物体/人物状態をまたぐ複数scene/cutの共通アンカーとして再利用する
